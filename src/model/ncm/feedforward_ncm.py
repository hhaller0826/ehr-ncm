...
class FF_NCM(SCM):
    def __init__(self, cg, *, v_size={}, default_v_size=1, u_size={},
                 default_u_size=1, f={}, hyperparams=None, scale={}, **kwargs):
    ...
    def save(self, path: str):
        T.save(self.state_dict(), _modelpath(path))
        print(f"Model saved to {path}")

    @staticmethod
    def load(path: str|None, cg, **kwargs):
       obj = FF_NCM(cg, **kwargs).to(DEVICE)
       if path is not None: 
           obj.load_state_dict(T.load(_modelpath(path)))
       return obj
    
def _modelpath(path): 
    if path.split('.')[-1] != "pth": 
        return f"out/models/{path}.pth"
    return path
